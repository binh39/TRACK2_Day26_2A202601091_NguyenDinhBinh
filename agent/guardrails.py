...
_INJECTION_PATTERNS = (
    ...
    r"you\s+must\s+now",
    r"reveal\s+the\s+instructor",
    r"reveal\s+(the\s+)?(act|scopes|grading\s+key|private)",
    r"grading\s+key",
...
_PRIVATE_PATTERNS = (
    r"failed\s+the\s+mid-term\s+assessment[^\n]*",
    r"Learner:\w+['’]s\s+private\s+note[^\n]*",
    r"private:\s*true[^\n]*",
    r"CONFIDENTIAL:[^\n]*",
    r"x{40,}[^\n]*",
)
...
def redact(text: str) -> RedactionResult:
    ...
    for pat in _PRIVATE_PATTERNS:
        matches = re.findall(pat, redacted, flags=re.IGNORECASE)
        for m in matches:
            if len(m) >= 40:
                hits.append(m)
...
def verify_arithmetic(text: str) -> ArithmeticCheckResult:
    ...
    if not numbers:
        return ArithmeticCheckResult(checked=True, ok=True, detail="no numbers found to check")
...
def abstention_policy(grounding: GroundingResult) -> bool:
    return not grounding.grounded or bool(grounding.ungrounded) or bool(grounding.malformed)
...
if __name__ == "__main__":
    ...
    print("\n=== agent.guardrails: security checks demo ===\n")
