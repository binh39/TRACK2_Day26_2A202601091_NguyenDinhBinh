...
class Gateway:
    ...
    def __init__(self, ctx: GatewayContext) -> None:
        ...
        self._admitted_cards: dict[str, dict] = {
            ...
            "roster": {"verified": True, "skills": ("get_learner", "list_cohort")},
        }
    ...
    def decide(self, cmd: Command) -> Decision:
        ...
        if self._instruction_in_content(cmd):
            self.quarantined += 1
            self.denied += 1
            return self.deny(cmd, "instruction found in retrieved content", quarantine=True)
