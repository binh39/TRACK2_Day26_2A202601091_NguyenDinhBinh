...
# Spend less while damage is x1.0 and preserve capacity for the x1.25/x1.5
# rounds.  The table sums to 95, leaving five credits for an unavoidable
# charged failure without making a normal ten-round plan impossible.
...
# A wildcard is never forwarded.  These masks retain the fields needed to
# validate/cite a result while dropping bulky optional payloads.  Explicit,
# valid narrow masks supplied by the model are preserved by ``safe_fields``.
_WILDCARD_MASKS: Mapping[tuple[str, str], tuple[str, ...]] = {
    ("slides", "query"): ("score", "title"),
    ("slides", "get_frame"): ("body", "confidence", "etag", "title"),
    ("registry", "list_servers"): ("name",),
    ("registry", "provenance"): ("etag", "rev"),
    ("glossary", "list_terms"): ("sense", "term"),
    ("citation-checker", "verify_source"): ("anchor", "confidence", "url"),
    ("curriculum-analyst", "which_days_cover"): ("anchor", "confidence", "course_day", "track"),
...
def safe_fields(server: str, tool: str, requested: tuple[str, ...]) -> tuple[str, ...]:
    """Return a valid, bounded field mask for one call.

    Unknown fields are rejected with ``ValueError`` so the gateway can deny a
    doomed charged request.  A bare mask keeps the tool's normal default,
    except for the two catalog traps whose bare default is deliberately the
    full expensive dump.  A wildcard is always replaced by a citation-useful
    narrow mask.
    """
    spec = TOOL_SPECS.get((server, tool)) if _SPECS_AVAILABLE else None
    canonical = tuple(sorted({str(f).lower() for f in requested}))
    if spec is None:
        if _SPECS_AVAILABLE:
    ...
    if canonical == ("*",):
        return _WILDCARD_MASKS.get((server, tool), tuple(spec.default_fields))
    if not canonical and (server, tool) in CATALOG_TRAP_TOOLS:
        return _WILDCARD_MASKS[(server, tool)]

    unknown = set(canonical) - set(spec.all_fields)
    if unknown:
        raise ValueError(f"unknown fields for {server}.{tool}: {sorted(unknown)}")
    return canonical


def bounded_args(server: str, tool: str, args: Mapping[str, Any]) -> dict[str, Any]:
    """Copy args and cap search fan-out to a useful, sustainable size."""
    out = dict(args)
    if (server, tool) in {("slides", "query"), ("slides", "search")}:
        limit = out.get("limit", 3)
        if not isinstance(limit, int) or isinstance(limit, bool) or limit < 1:
            raise ValueError(f"{server}.{tool} limit must be a positive int")
        out["limit"] = min(limit, 3)
    return out
...
def estimate_call_cost(
    ...
    """Conservative pre-dispatch cost estimate used by the gateway.

    Row-priced search uses its bounded ``limit`` as the upper bound; all
    other shipped tools have row_weight zero.  Unknown tools are assigned a
    deliberately unaffordable cost instead of being treated as free.
    """
    ...
    n_rows = 0
    if spec.row_weight:
        raw_limit = (args or {}).get("limit", 3)
        n_rows = raw_limit if isinstance(raw_limit, int) and not isinstance(raw_limit, bool) else 3
        n_rows = max(0, min(n_rows, 3))
    return int(_spec_cost(server, tool, fields=fields, n_rows=n_rows))


def round_allowance(round_no: int) -> int:
    """Credit allowance for a round; unknown round numbers get the median."""
    return ROUND_ALLOWANCES.get(round_no, 9)


def future_reserve(round_no: int) -> int:
    """Credits reserved for all rounds strictly after ``round_no``."""
    return sum(v for r, v in ROUND_ALLOWANCES.items() if r > round_no)
